...
<table class="table table-striped">
  ...
  <tbody>
  	<c:forEach items="${list}" var="reportVO" varStatus="status">
							...
							<td>${reportVO.time}</td>
							<td>
							<a href="<%=request.getContextPath() %>/dealreportController/edit.action?id=${reportVO.id}">编辑</a>
							<a href="<%=request.getContextPath() %>/report/doDelete.action?id=${reportVO.id}">删除</a>
							</td>
							</tr>
	</c:forEach> 
  </tbody>
